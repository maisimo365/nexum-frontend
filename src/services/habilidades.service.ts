...
export async function updatePortfolioSkill(
  ...
  const res = await apiFetch<ApiPortfolioSkillResponse>(
    `/portfolio/skills/${portfolioSkillId}`,
...
export async function suggestSkill(
  ...
  const body: { name: string; type: string; category: string; description?: string } = { name, type, category };
